function main() {
  ...
  Operate.newInput(["a1", "a2", "b1", "b2", "c1", "c2"]).then(
    (coefficients) => {
      ...
      if (a2 != 0 && b1 - (a1 / a2) * b2 != 0) {
        ...
        const x = (c2 - b2 * y) / a2;
        solutions = [`x = ${x.toFixed(2)}`, `y = ${y.toFixed(2)}`];
      ...
      if (a2 != 0 && b1 - (a1 / a2) * b2 != 0) {
        Operate.newStart(
          "Calculate y using the system of linear equations formula: "
        );
        ...
        const step5 = `Solution: x = ${xDecimal.toFixed(
          2
        )}\\ or\\ y = ${yDecimal.toFixed(2)}`;
