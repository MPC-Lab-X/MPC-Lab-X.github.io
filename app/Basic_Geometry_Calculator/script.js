function main() {
  ...
  Operate.newInput(["shape name"]).then((shapeInput) => {
...
function continueWithShape(shapeName) {
  switch (shapeName) {
    case "square":
      Operate.newInput(["operation"]).then((operationInput) => {
        const operation = operationInput[0].toLowerCase(); // Convert to lowercase for case-insensitivity
        if (operation === "perimeter") {
          Operate.newInput(["do you have all the side lengths? (Y/N)"]).then(
            (question1) => {
              const answer1 = question1[0].toLowerCase();
              if (answer1 === "y") {
                Operate.newInput(["sides (comma-separated)"]).then(
                  (sideInput) => {
                    ...
                    if (shapeName === "square" && sides.length === 1) {
                      ...
                      Operate.newSolution(
                        "Invalid \\ input \\ for \\ the \\ selected \\ shape."
                      );
                      // Add a reset button
                      Operate.newRestart();
                    }
                ...
                Operate.newInput(["Do you have the diagonal? (Y/N)"]).then(
                  (question2) => {
                    const answer2 = question2[0].toLowerCase();
                    if (answer2 === "y") {
                      Operate.newInput(["diagonal length"]).then(
                        (diagonalInput) => {
                          const diagonal = parseFloat(diagonalInput[0]);
                          if (shapeName === "square") {
                            squarePerimeterCalculationWithDiagonal(
                              shapeName,
                              diagonal
                            );
                          } else {
          ...
          Operate.newInput(["do you have all the side lengths? (Y/N)"]).then(
            (question1) => {
              ...
              if (answer1 === "y") {
                if (shapeName === "square") {
                  Operate.newInput(["side length"]).then((sideInput) => {
                    ...
                    if (answer2 === "y") {
                      Operate.newInput(["diagonal length"]).then(
                        (diagonalInput) => {
                          const diagonal = parseFloat(diagonalInput[0]);
                          if (shapeName === "square") {
                            squareAreaCalculationWithDiagonal(
                              shapeName,
                              diagonal
                            );
                          } else {
    ...
    case "rectangle":
      Operate.newInput(["operation"]).then((operationInput) => {
        const operation = operationInput[0].toLowerCase(); // Convert to lowercase for case-insensitivity
        if (operation === "perimeter") {
          Operate.newInput(["Do you have the width and length? (Y/N)"]).then(
            (question1) => {
              const answer1 = question1[0].toLowerCase();
              if (answer1 === "y") {
                Operate.newInput(["sides (comma-separated)"]).then(
                ...
                Operate.newInput(["Do you have width or length or none?"]).then(
                  (question2) => {
                    const answer2 = question2[0].toLowerCase();
                    if (answer2 === "width" || answer2 === "length") {
                      Operate.newInput([
                        "Do you have the diagonal length? (Y/N)",
                      ]).then((question3) => {
                        const answer3 = question3[0].toLowerCase();
                        if (answer3 === "y") {
                          Operate.newInput(["side length"]).then(
                            (sideInput) => {
                              var side = parseFloat(sideInput[0]);
                              Operate.newInput(["diagonal length"]).then(
                                (diagonalInput) => {
                                  const diagonal = parseFloat(diagonalInput[0]);
                                  rectanglePerimeterCalculationWithDiagonal(
                                    shapeName,
                                    diagonal,
                                    side
                                  );
                          ...
                          Operate.newInput([
                            "Do you have the angle between the between the diagonal and one side? (Y/N)",
                          ]).then((question4) => {
                            ...
                            if (answer4 === "y") {
                              Operate.newInput(["side length"]).then(
                                (sideInput) => {
                                  var side = parseFloat(sideInput[0]);
                                  Operate.newInput(["angle"]).then(
                                    (angleInput) => {
                                      const angle = parseFloat(angleInput[0]);
                                      rectanglePerimeterCalculationWithAngle(
                                        shapeName,
                                        side,
                                        angle
                                      );
          ...
          Operate.newInput(["do you have all the side lengths? (Y/N)"]).then(
            (question1) => {
              const answer1 = question1[0].toLowerCase();
              if (answer1 === "y") {
                Operate.newInput(["length", "width"]).then(
                  (dimensionsInput) => {
                          ...
                          Operate.newInput(["side length"]).then(
                            (sideInput) => {
                              ...
                              Operate.newInput(["diagonal length"]).then(
                                (diagonalInput) => {
                                  const diagonal = parseFloat(diagonalInput[0]);
                                  rectangleAreaCalculationWithDiagonal(
                                    shapeName,
                                    diagonal,
                                    side
                                  );
                              ...
                              Operate.newInput(["side length"]).then(
                                (sideInput) => {
                                  ...
                                  Operate.newInput(["angle"]).then(
                                    (angleInput) => {
                                      const angle = parseFloat(angleInput[0]);
                                      rectangleAreaCalculationWithAngle(
                                        shapeName,
                                        side,
                                        angle
                                      );
    ...
    case "trapezoid":
      Operate.newInput(["operation"]).then((operationInput) => {
        ...
        if (operation === "area") {
          Operate.newInput(["top width", "bottom width", "height"]).then(
            (dimensionsInput) => {
          ...
          Operate.newInput([
            ...
            const [width1, width2, length1, length2] =
              dimensionsInput.map(parseFloat);
            trapezoidPerimeterCalculation(
              shapeName,
              width1,
              width2,
              length1,
          ...
          Operate.newSolution(
            "Invalid \\ operation \\ for \\ the \\ selected \\ shape."
          );
          // Add a reset button
          Operate.newRestart();
        }
    ...
    case "prism":
      Operate.newInput(["base shape"]).then((baseInput) => {
        const base = baseInput[0].toLowerCase();
        if (base === "rectangular" || base === "rectangle") {
          Operate.newInput(["operation"]).then((operationInput) => {
            const operation = operationInput[0].toLowerCase(); // Convert to lowercase for case-insensitivity
            if (operation === "volume") {
              Operate.newInput(["length", "width", "height"]).then(
                (dimensionsInput) => {
                  const [length, width, height] =
                    dimensionsInput.map(parseFloat);
                  rectangularPrismVolumeCalculation(
                    shapeName,
                    length,
                    width,
                    height
              ...
              Operate.newInput(["length", "width", "height"]).then(
                (dimensionsInput) => {
                  const [length, width, height] =
                    dimensionsInput.map(parseFloat);
                  rectangularPrismSurfaceAreaCalculation(
                    shapeName,
                    length,
                    width,
                    height
          ...
          Operate.newInput(["operation"]).then((operationInput) => {
            ...
            if (operation === "surface area") {
              Operate.newInput([
                ...
                const [width, height, length1, length2] =
                  radiusInput.map(parseFloat);
                triangularPrismSurfaceAreaCalculation(
                  shapeName,
                  width,
                  length1,
                  length2,
              ...
              Operate.newInput(["length", "base height", "width"]).then(
                (radiusInput) => {
                  const [length, height, width] = radiusInput.map(parseFloat);
                  triangularPrismVolumeCalculation(
                    shapeName,
                    width,
                    length,
                    height
          ...
          Operate.newInput(["operation"]).then((operationInput) => {
            ...
            if (operation === "surface area") {
              Operate.newInput(["apothem", "side length", "slantHeight"]).then(
                (radiusInput) => {
                  const [apothem, baseLength, slantHeight] =
                    radiusInput.map(parseFloat);
                  hexagonalPrismSurfaceAreaCalculation(
                    shapeName,
                    apothem,
                    baseLength,
                    slantheight
              ...
              Operate.newInput(["side length", "height", "apothem"]).then(
                (radiusInput) => {
                  const [baseLength, height, apothem] =
                    radiusInput.map(parseFloat);
                  hexagonalPrismVolumeCalculation(
                    shapeName,
                    baseLength,
                    apothem,
                    height
          ...
          Operate.newSolution(
            "Sorry \\ we \\ currently \\ don't \\ have \\ the \\ equation \\ for \\ this \\ base."
          );
          // Add a reset button
          Operate.newRestart();
        }
    ...
    case "triangle":
      Operate.newInput(["operation"]).then((operationInput) => {
        ...
        if (operation === "perimeter") {
          Operate.newInput([
            "How many sides of the triangle do you have?",
          ]).then((question1) => {
            const answer1 = question1[0].toLowerCase();
            if (answer1 === "3") {
              Operate.newInput(["side1", "side2", "side3"]).then(
                (sideInput) => {
              ...
              Operate.newInput(["side1", "side2"]).then((sideInput) => {
                const side1 = parseFloat(sideInput[0]);
                const side2 = parseFloat(sideInput[1]);
                trianglePerimeterCalculationWithTwoSide(
                  shapeName,
                  side1,
                  side2
                );
              ...
              Operate.newSolution(
                "Sorry \\ we \\ can't \\ solve \\ this \\ one \\ yet."
              );
              // Add a reset button
              Operate.newRestart();
            } else {
          ...
          Operate.newInput(["Do you have the base and height? (Y/N)"]).then(
            (question1) => {
              const answer1 = question1[0].toLowerCase();
              if (answer1 === "y") {
                Operate.newInput(["base", "height"]).then((dimensionsInput) => {
                  const [base, height] = dimensionsInput.map(parseFloat);
                  triangleAreaCalculation(shapeName, base, height);
                });
              } else if (answer1 === "n") {
                Operate.newInput([
                  "Do you have the all the sides of the triangle? (Y/N)",
                ]).then((question2) => {
                  const answer2 = question2[0].toLowerCase();
                  if (answer2 === "y") {
                    Operate.newInput(["side1", "side2", "side3"]).then(
                      (sideInput) => {
                        ...
                        triangleAreaCalculationWithAllSide(
                          shapeName,
                          side1,
                          side2,
                          side3
                    ...
                    Operate.newInput([
                      "Is the triangle inscribe in a circle? (Y/N)",
                    ]).then((question3) => {
                      const answer3 = question3[0].toLowerCase();
                      if (answer3 === "y") {
                        Operate.newInput([
                          "Do you have the angle between the radius from one of the vertex and the diameter? (Y/N)",
                        ]).then((question4) => {
                          const answer4 = question4[0].toLowerCase();
                          if (answer4 === "y") {
                            Operate.newInput(["radius", "angle"]).then(
                              (dimensionsInput) => {
                                const radius = parseFloat(dimensionsInput[0]);
                                const angle = parseFloat(dimensionsInput[1]);
                                triangleAreaCalculationWithRadius(
                                  shapeName,
                                  radius,
                                  angle
                                );
    ...
    case "circle":
      Operate.newInput(["operation"]).then((operationInput) => {
        ...
        if (operation === "circumference") {
          Operate.newInput(["radius"]).then((radiusInput) => {
    ...
    case "oval":
      Operate.newInput(["operation"]).then((operationInput) => {
        ...
        if (operation === "area") {
          Operate.newInput(["long radius", "short radius"]).then(
            (radiusInput) => {
    ...
    case "cylinder":
      Operate.newInput(["operation"]).then((operationInput) => {
        ...
        if (operation === "surface area") {
          Operate.newInput(["radius, height"]).then((radiusInput) => {
    ...
    case "cone":
      Operate.newInput(["operation"]).then((operationInput) => {
        ...
        if (operation === "surface area") {
          Operate.newInput(["radius", "slantHeight"]).then((radiusInput) => {
      ...
      Operate.newInput(["base shape"]).then((baseInput) => {
        ...
        if (base == "square") {
          Operate.newInput(["operation"]).then((operationInput) => {
            ...
            if (operation === "surface area") {
              Operate.newInput(["base width", "slantHeight"]).then(
                (radiusInput) => {
                  const [width, slantHeight] = radiusInput.map(parseFloat);
                  squarePyramidSurfaceAreaCalculation(
                    shapeName,
                    width,
                    slantHeight
                  );
              ...
              Operate.newInput(["base width", "height"]).then((radiusInput) => {
        ...
        if (base == "rectangle" || base == "rectangular") {
          Operate.newInput(["operation"]).then((operationInput) => {
            ...
            if (operation === "surface area") {
              Operate.newInput(["width", "length", "slantHeight"]).then(
                (radiusInput) => {
                  const [width, length, slantHeight] =
                    radiusInput.map(parseFloat);
                  rectanglePyramidSurfaceAreaCalculation(
                    shapeName,
                    width,
                    length,
                    height
              ...
              Operate.newInput(["width", "length", "height"]).then(
                (radiusInput) => {
                  const [width, length, height] = radiusInput.map(parseFloat);
                  rectanglePyramidVolumeCalculation(
                    shapeName,
                    width,
                    length,
                    height
          ...
          Operate.newInput(["operation"]).then((operationInput) => {
            ...
            if (operation === "surface area") {
              Operate.newInput([
                ...
                const [width, height, length1, length2, slantHeight] =
                  radiusInput.map(parseFloat);
                triangularPyramidSurfaceAreaCalculation(
                  shapeName,
                  width,
                  length1,
                  length2,
              ...
              Operate.newInput(["width", "base height", "height"]).then(
                (radiusInput) => {
                  const [width, length, height] = radiusInput.map(parseFloat);
                  triangularPyramidVolumeCalculation(
                    shapeName,
                    width,
                    length,
                    height
            ...
            if (operation === "surface area") {
              Operate.newInput(["apothem", "side length", "slantHeight"]).then(
                (radiusInput) => {
                  const [apothem, baseLength, slantHeight] =
                    radiusInput.map(parseFloat);
                  pentagonalPyramidSurfaceAreaCalculation(
                    shapeName,
                    apothem,
                    baseLength,
                    slantheight
              ...
              Operate.newInput(["apothem", "side length", "height"]).then(
                (radiusInput) => {
                  const [apothem, baseLength, height] =
                    radiusInput.map(parseFloat);
                  pentagonalPyramidVolumeCalculation(
                    shapeName,
                    apothem,
                    baseLength,
                    height
        ...
        if (base == "hexagon" || base == "hexagonal") {
            ...
            if (operation === "surface area") {
              Operate.newInput(["apothem", "side length", "slantHeight"]).then(
                (radiusInput) => {
                  const [apothem, baseLength, slantHeight] =
                    radiusInput.map(parseFloat);
                  hexagonalPyramidSurfaceAreaCalculation(
                    shapeName,
                    apothem,
                    baseLength,
                    slantheight
              ...
              Operate.newInput(["side length", "height"]).then(
                (radiusInput) => {
                  const [baseLength, height] = radiusInput.map(parseFloat);
                  hexagonalPyramidVolumeCalculation(
                    shapeName,
                    baseLength,
                    height
                  );
          ...
          Operate.newSolution(
            "Sorry \\ we \\ don't \\ have \\ the \\ equation \\ for \\ this \\ base."
          );
          // Add a reset button
          Operate.newRestart();
        }
    ...
    default:
      ...
      if (closestMatch) {
        Operate.newSolution(
          "Invalid \\ shape \\ name. \\ Did \\ you \\ mean \\ " +
            closestMatch +
            "?"
        );
      } else {
...
function squarePerimeterCalculation(shapeName, sides) {
  ...
  Operate.newSolution(
    `The \\ perimeter \\ of \\ the \\ ${shapeName} \\ is: \\ ${perimeter}`
  );
  // Add a reset button
  Operate.newRestart();
}
...
function squareAreaCalculation(shapeName, side) {
  ...
  Operate.newSolution(
    `The \\ area \\ of \\ the \\ ${shapeName} \\ is: \\ ${area}`
  );
  // Add a reset button
  Operate.newRestart();
}
...
// ---------Trapezoid-------------
function trapezoidPerimeterCalculation(
  shapeName,
  width1,
  width2,
  length1,
...
function cubeVolumeCalculation(shapeName, side) {
  ...
  Operate.newSolution(
    `The \\ volume \\ of \\ the \\ ${shapeName} \\ is: \\ ${volume}`
  );
  // Add a reset button
  Operate.newRestart();
}
...
function cubeSurfaceAreaCalculation(shapeName, side) {
  ...
  Operate.newSolution(
    `The \\ surface \\ area \\ of \\ the ${shapeName} \\ is: \\ ${surfaceArea}`
  );
  // Add a reset button
  Operate.newRestart();
}
...
function rectangularPrismSurfaceAreaCalculation(
  shapeName,
  length,
  width,
  height
...
function triangularPrismSurfaceAreaCalculation(
  shapeName,
  width,
  length1,
  length2,
...
// ---------Hexagonal-------------
function hexagonalPrismVolumeCalculation(
  shapeName,
  baseLength,
  height,
  apothem
...
function hexagonalPrismSurfaceAreaCalculation(
  shapeName,
  length,
  apothem,
  height
...
function triangleAreaCalculationWithRadius(shapeName, radius, angle) {
  const area =
    ((radius + radius * Math.cos(angle)) * (2 * radius * Math.sin(angle))) / 2;
  Operate.newSolution(
    `The \\ area \\ of \\ the \\ ${shapeName} \\ is: \\ ${area}`
  );
  // Add a reset button
...
function circleCircumferenceCalculation(shapeName, radius) {
  ...
  Operate.newSolution(
    `The \\ circumference \\ of \\ the \\ ${shapeName} \\ is: \\ ${circumference}`
  );
  // Add a reset button
  Operate.newRestart();
}
...
function coneSurfaceAreaCalculation(shapeName, radius, slantHeight) {
  const surfaceArea =
    Math.PI * radius * radius + slantHeight * radius * Math.PI;
  Operate.newSolution(
    `The \\ surface \\ area \\ of \\ the \\ ${shapeName} \\ is: \\ ${surfaceArea}`
  );
  // Add a reset button
...
function rectanglePyramidSurfaceAreaCalculation(
  shapeName,
  width,
  length,
  height
  ...
  const surfaceArea =
    length * width +
    0.5 * width * Math.sqrt(4 * height * height + length * length) +
    0.5 * length * Math.sqrt(4 * height * height + length * length);
  Operate.newSolution(
    `The \\ surface \\ area \\ of \\ the \\ ${shapeName} \\ is: \\ ${surfaceArea}`
  );
  // Add a reset button
...
// ---------Triangle-------------
function triangularPyramidSurfaceAreaCalculation(
  shapeName,
  width,
  length1,
  length2,
  ...
  const surfaceArea =
    0.5 * (width + length1 + length2) * slantHeight + 0.5 * width * height;
  Operate.newSolution(
    `The \\ surface \\ area \\ of \\ the \\ ${shapeName} \\ is: \\ ${surfaceArea}`
  );
  // Add a reset button
...
// ---------Pentagon-------------
function pentagonalPyramidSurfaceAreaCalculation(
  shapeName,
  apothem,
  base,
  slantHeight
...
// ---------Hexagonal-------------
function hexagonalPyramidSurfaceAreaCalculation(
  shapeName,
  apothem,
  base,
  slantHeight
