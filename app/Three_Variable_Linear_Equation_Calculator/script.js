function main() {
  ...
  Operate.newStart(
    "Please enter coefficients a1, a2, a3, b1, b2, b3, c1, c2, c3, d1, d2, d3:"
  );
  ...
  Operate.newInput([
    ...
    const [a1, a2, a3, b1, b2, b3, c1, c2, c3, d1, d2, d3] =
    ...
    if (det === 0) {
      ...
      const x =
        (d1 * b2 * c3 - c2 * b3 * d1 - b1 * d2 * c3 + c1 * d2 * b3) / det;
      Operate.newStep(
        `x = \\frac{${
          d1 * b2 * c3 - c2 * b3 * d1 - b1 * d2 * c3 + c1 * d2 * b3
        }}{{${det}}}`
      );
      Operate.newStep(`x = ${x}`);

      const y =
        (a1 * d2 * c3 - c2 * d3 * a1 - d1 * a2 * c3 + c1 * a2 * d3) / det;
      Operate.newStep(
        `y = \\frac{${
          a1 * d2 * c3 - c2 * d3 * a1 - d1 * a2 * c3 + c1 * a2 * d3
        }}{{${det}}}`
      );
      Operate.newStep(`y = ${y}`);

      const z =
        (a1 * b2 * d3 - d2 * b3 * a1 - b1 * a2 * d3 + d1 * a2 * b3) / det;
      Operate.newStep(
        `z = \\frac{${
          a1 * b2 * d3 - d2 * b3 * a1 - b1 * a2 * d3 + d1 * a2 * b3
        }}{{${det}}}`
      );
      Operate.newStep(`z = ${z}`);
